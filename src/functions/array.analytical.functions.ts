...
export class ArrayAnalyticalFunction extends NumericFunctionExpression {
    ...
    public expectsParameters(): TypedParameter[] {
        switch (this.name) {
            ...
            default:
                throw new TypeCheckError(`Unknown array analytical function: ${this.name}`);
        }
    }
    ...
    public evaluate(context: WorkingContext): number {
        ...
        if (!Array.isArray(targetValue)) {
            ...
            throw new EvaluationError(`Target argument for function ${this.name} did not evaluate to an array`, { providedArgs: [this.target_arg] });
        }
        ...
        if (!Array.isArray(compareTo)) {
            ...
            throw new EvaluationError(`Compare-to argument for function ${this.name} did not evaluate to an array`, { providedArgs: [this.extra_args[0]] });
        }
        if (targetValue.length !== compareTo.length) {
            if (!['jaccard_distance', 'cross_correlation', 'kolmogorov_smirnov_distance', 'earth_movers_distance', 'wasserstein_distance'].includes(this.name)) {
                ...
                context.logger().warn(message);
                context.addException(new EvaluationError(message, { targetValue, compareTo }));
                return NaN;
                // throw new EvaluationError(message, { targetValue, compareTo });
            }
        }
        ...
        switch (this.name) {
            ...
            default:
                throw new EvaluationError(`Unknown array analytical function: ${this.name}`);
        }
    }
...
export class ArrayAnalyticalFunctionProvider {
    ...
    public static create(name: string, args: Expression[]): FunctionExpression | undefined {
        ...
        return new ArrayAnalyticalFunction(name, args[0]!, args.slice(1));
    }
