...
describe('Cloning tests', () => {
  ...
  it('handles workspace cloning safely', async () => {
    ...
    expect(Object.keys(space.getTypeMemory().getRootTypes()).length).toBe(1);
    expect(Object.keys(space.getFunctionMemory().getFunctions()).length).toBe(1);
    ...
    expect(Object.keys(clonedSpace.getTypeMemory().getRootTypes()).length).toBe(1);
    expect(Object.keys(clonedSpace.getFunctionMemory().getFunctions()).length).toBe(1);
    ...
    space.getTypeMemory().addRootType({ key: 'Animal', properties: { species: 'string' } });
    const triple = new FunctionParser({ workspace: space }).parse('triple(n: number) = n * 3');
    space.getFunctionMemory().addFunction(triple!);

    expect(space.checkTypes().valid).toBe(true);
    expect(space.getRules().length).toBe(3);
    expect(Object.keys(space.getTypeMemory().getRootTypes()).length).toBe(2);
    expect(Object.keys(space.getFunctionMemory().getFunctions()).length).toBe(2);
